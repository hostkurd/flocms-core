...
class ErrorHandler
{
    ...
    {
        try {
            if (method_exists(App::class, 'logger') && App::logger()) {
            ...
            try {
                $fallback = Template::getErrorPath('500.html');
                echo (new View([
    ...
    private static function renderDebugPage(Throwable $e, int $status = 500): void
    {
        ...
        $fileEsc = self::e($sourceFile);
        [$codeHtml, $codeTitle] = self::renderCodeContext($sourceFile, $line, 6);
        ...
        $templateFile = Template::getErrorPath('debug.html');
    ...
    private static function collectRequestInfo(): array
    {
        ...
        $router = method_exists(App::class, 'getRouter') ? App::getRouter() : null;
        ...
        return [
            ...
            'Language'   => $router ? ($router->getLanguage() ?? '') : '',
        ];
    }
